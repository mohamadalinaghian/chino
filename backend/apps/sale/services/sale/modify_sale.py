...
class ModifySaleService:
    ...
    @transaction.atomic
    def sync_items(
        *, sale: Sale, items_payload: List[OpenSaleService.ItemInput], performer
    ) -> Sale:
        # 1. Validation
        ...
        menu_ids = {item.menu.pk for item in items_payload}
        product_ids = {
            extra.product.pk for item in items_payload for extra in item.extras
        }
        ...
        incoming_ids = {
            item.menu.pk for item in items_payload if item.menu.pk is not None
        }
        ...
        for item_data in items_payload:
            ...
            # --- CASE B: CREATE NEW ---
                ...
                menu_obj = menus_map[item_data.menu.pk]

                extra_dtos = []
                for extra_data in item_data.extras:
                    if extra_data.product.pk not in products_map:
                        raise ValidationError(
                            f"Invalid Product ID: {extra_data.product.pk}"
                        )

                    extra_dtos.append(
                        OpenSaleService.ExtraInput(
                            product=products_map[extra_data.product.pk],
